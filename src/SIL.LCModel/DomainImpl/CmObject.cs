...
namespace SIL.LCModel.DomainImpl
{
	...
	/// </summary>
	/// <remarks>
	/// The GeneratedClasses.cs file contains the generated
	...
	internal partial class CmObject : ICmObjectInternal, ICmObjectOrIdInternal, ICmObjectOrSurrogate, IReferenceSource
	{
		...
		/// <summary>
		/// Contains all known CmObjects with atomic refs to this object, and all, LcmReferenceCollections and LcmReferenceSequences
		...
		/// initialized properties, since they can change during LCM reads.
		/// </summary>
		...
		/// <summary>
		/// This name is used in the LcmBrowser. It needs to identify objects unambiguously.
		/// </summary>
			...
			foreach (int flid in flidList)
			{
				...
				*/
				...
				if (fieldname == "DateCreated"
					|| nType == (int)CellarPropertyType.Image // LCM does not support this one.
					|| nType == (int)CellarPropertyType.GenDate) // LCM does not support setter for gendate.
				...
				switch (nType)
				{
					...
					case (int)CellarPropertyType.Integer: // 2 Fall through
						// Setter not implemented in LCM. case (int)CellarPropertyType.GenDate: // 8
						{
		...
		/// <param name="objNew"></param>
		/// <param name="objOld"></param>
		internal static void ReplaceReferencesWhereValid(LcmCache cache, ICmObject objOld, ICmObject objNew)
		{
			var cmObject = ((CmObject)objOld);
			cmObject.EnsureCompleteIncomingRefs();
			// FWR-2969 If merging senses, m_incomingRefs will sometimes get changed
		...
		internal void SetNonModelPropertyForSDA(int flid, object newValue, bool useAccessor)
		{
			...
			{
				throw new LcmInvalidFieldException("'flid' is not in the metadata cache, or 'flid' does not match the type of data used. Use an SDA decorator for your madeup property.", err);
			}
		}
		...
		protected virtual void SetVirtualPropertyForSDA(int flid, object newValue, bool useAccessor)
		{
			...
			switch ((CellarPropertyType)flidType)
			{
				...
				case CellarPropertyType.ReferenceSequence:
					...
					{
						((ILcmVectorInternal)vectorRetval).ReplaceAll((Guid[])newValue, true);
					}
					break;
			}
		}
		...
		internal IEnumerable<ICmObject> GetNonModelPropertyForSdaAsEnumerable(int flid)
		{
			...
			return (retval as IEnumerable<ICmObject>) ?? ((ILcmVector)retval).Objects;
		}
		...
		internal object GetNonModelPropertyForSDA(int flid)
		{
			...
			{
				throw new LcmInvalidFieldException("'flid' is not in the metadata cache. Use an SDA decorator for your madeup property.", err);
			}
		}
		...
		/// </summary>
		/// <param name="cache"></param>
		void ICmObjectInternal.InitializeNewOwnerlessCmObject(LcmCache cache)
		{
		...
		{
			if (m_cache == null)
				throw new LcmCacheUnusableException("There is no LcmCache for this object.");
			if (m_cache.IsDisposed)
				throw new LcmCacheUnusableException("The LcmCache has been disposed, so this object cannot be used.");
			if (m_hvo == (int)SpecialHVOValues.kHvoUninitializedObject)
				throw new LcmObjectUninitializedException("Object has no valid HVO.");
			if (m_hvo == (int)SpecialHVOValues.kHvoObjectDeleted)
				throw new LcmObjectDeletedException("Object has been deleted, and cannot be used.");
		}
		...
		/// ------------------------------------------------------------------------------------
		void ICmObjectInternal.ResetForUndoRedo(LcmCache cache, int newHvo)
		{
			m_cache = cache;
			m_hvo = newHvo;
		}
		...
		private void ValidateOwnership(ICmObject owner, int owningFlid)
		{
			...
			if (Hvo == (int)SpecialHVOValues.kHvoUninitializedObject)
				throw new LcmObjectUninitializedException("Owned object has no valid HVO.");
			if (Hvo == (int)SpecialHVOValues.kHvoObjectDeleted)
				throw new LcmObjectDeletedException("Owned object has been deleted.");
			if (owner == null)
				throw new ArgumentNullException("owner");
			if (owner.Hvo == (int)SpecialHVOValues.kHvoUninitializedObject)
				throw new LcmObjectUninitializedException("Owning object has no valid HVO.");
			if (owner.Hvo == (int)SpecialHVOValues.kHvoObjectDeleted)
				throw new LcmObjectDeletedException("Owning object has been deleted.");
		...
		public void Delete()
		{
			...
			else
			{
				...
				// Use SetProperty/Replace to remove it
				int tag = OwningFlid;
				if(tag == 0)
				{
					throw new LcmInvalidFieldException(String.Format("Error when deleting object of class {0}, we were not found in our owner {1}.", goner.GetType().FullName, owner.GetType().FullName));
				}
		...
		/// </summary>
		void ICmObjectInternal.LoadFromDataStore(LcmCache cache, XElement reader, LoadingServices loadingServices)
		{
			if (cache == null) throw new ArgumentNullException("cache");
		...
		protected virtual int GetObjectPropertyInternal(int flid)
		{
			switch (flid)
			{
				...
				case (int)CmObjectFields.kflidCmObject_Owner:
					return Owner == null ? LcmCache.kNullHvo : Owner.Hvo;
			}
		}
		...
		protected virtual void SetPropertyInternal(int flid, Guid[] newValue, bool useAccessor)
		{
			if (m_cache.ServiceLocator.GetInstance<IFwMetaDataCacheManaged>().IsCustom(flid))
			{
				var vector = GetNonModelPropertyForSDA(flid) as ILcmVectorInternal;
				if (vector == null)
		...
		protected virtual void ReplaceInternal(int flid, int start, int numberToDelete, IEnumerable<ICmObject> thingsToAdd)
		{
			...
			if (seq is ILcmList<ICmObject>)
				((ILcmList<ICmObject>)seq).Replace(start, numberToDelete, thingsToAdd);
			else if (seq is ILcmList<ICmPossibility>)
				((ILcmList<ICmPossibility>)seq).Replace(start, numberToDelete, thingsToAdd);
			else
		...
		protected virtual void DeleteObjectBasics()
		{
			...
			foreach (var flid in mdc.GetFields(ClassID, true, (int)(CellarPropertyTypeFilter.AllOwning | CellarPropertyTypeFilter.AllReference)))
			{
				...
				switch (dataType)
				{
					...
					case CellarPropertyType.OwningCollection:
						// Delete objects.
						((LcmOwningCollection<ICmObject>)data).Clear();
						break;
					case CellarPropertyType.OwningSequence:
						// Delete objects.
						((LcmOwningSequence<ICmObject>)data).Clear();
						break;
					case CellarPropertyType.ReferenceCollection:
						((LcmReferenceCollection<ICmObject>)data).Clear();
						break;
					case CellarPropertyType.ReferenceSequence:
						((LcmReferenceSequence<ICmObject>)data).Clear();
						break;
				}
			...
			foreach (var flid in mdc.GetFields(ClassID, true, (int)CellarPropertyTypeFilter.AllReference))
			{
				...
				switch (dataType)
				{
					...
					case CellarPropertyType.ReferenceAtomic:
						...
						break;
					case CellarPropertyType.ReferenceCollection:
						((LcmReferenceCollection<ICmObject>)data).ClearForUndo();
						break;
					case CellarPropertyType.ReferenceSequence:
						((LcmReferenceSequence<ICmObject>)data).ClearForUndo();
						break;
				}
			}
		}
			...
			foreach (var flid in mdc.GetFields(ClassID, true, (int)CellarPropertyTypeFilter.AllReference))
			{
				...
				switch (dataType)
				{
					...
					case CellarPropertyType.ReferenceAtomic:
						...
						break;
					case CellarPropertyType.ReferenceCollection:
						((LcmReferenceCollection<ICmObject>)data).RestoreAfterUndo();
						break;
					case CellarPropertyType.ReferenceSequence:
						((LcmReferenceSequence<ICmObject>)data).RestoreAfterUndo();
						break;
				}
			}
		}
		...
		protected virtual object GetVirtualPropertyForSDA(int flid)
		{
			...
			switch ((CellarPropertyType)flidType)
			{
				...
				case CellarPropertyType.ReferenceSequence:
					...
					// Read callers know how to get Objects from ILcmVector. Write callers may try casting
					// to another type, in case we want to handle writeable virtuals.
					if (retval is IEnumerable<ICmObject> || retval is ILcmVector)
						return retval;
		...
		private void CheckLegalFlidForObject(int flid)
		{
			...
			if (!flids.Contains(flid))
				throw new LcmInvalidFieldException("'flid' is not defined for this class. Use an SDA decorator for your made up property.");
		}
		...
		private string CheckVirtualProperty(int flid, out int flidType)
		{
			...
			try
			{
				...
				if (!isVirtualProperty)
					throw new LcmInvalidFieldException("'flid' must be a virtual field to be used in this method.");
			...
			{
				throw new LcmInvalidFieldException("'flid' is not in the metadata cache. Use an SDA decorator for your madeup property.");
			}
		}
			...
			if (retval == null)
			{
				...
				switch (dataType)
				{
					...
					case CellarPropertyType.MultiUnicode:
						...
						break;
					case CellarPropertyType.OwningCollection:
						retval = new LcmOwningCollection<ICmObject>(
							((IServiceLocatorInternal)Cache.ServiceLocator).UnitOfWorkService,
							Cache.ServiceLocator.GetInstance<ICmObjectRepository>(),
							this, flid);
						break;
					case CellarPropertyType.OwningSequence:
						retval = new LcmOwningSequence<ICmObject>(
							((IServiceLocatorInternal)Cache.ServiceLocator).UnitOfWorkService,
							Cache.ServiceLocator.GetInstance<ICmObjectRepository>(),
							this, flid);
						break;
					case CellarPropertyType.ReferenceCollection:
						retval = new LcmReferenceCollection<ICmObject>(
							((IServiceLocatorInternal)Cache.ServiceLocator).UnitOfWorkService,
							Cache.ServiceLocator.GetInstance<ICmObjectRepository>(),
							this, flid);
						break;
					case CellarPropertyType.ReferenceSequence:
						retval = new LcmReferenceSequence<ICmObject>(
							((IServiceLocatorInternal)Cache.ServiceLocator).UnitOfWorkService,
							Cache.ServiceLocator.GetInstance<ICmObjectRepository>(),
							this, flid);
						break;
		...
		private void UserDefinedProperties(XmlWriter writer)
		{
			...
			foreach (var flid in mdc.GetFields(ClassID, true, (int)CellarPropertyTypeFilter.All))
			{
				...
				switch (dataType)
				{
					...
					case CellarPropertyType.ReferenceCollection: // Fall through.
					case CellarPropertyType.ReferenceSequence:
						var refVector = (ILcmVectorInternal)data;
			...
			foreach (var customPropertyElement in customElements)
			{
				...
				switch (dataType)
				{
					...
					case CellarPropertyType.MultiUnicode:
						...
						break;
					case CellarPropertyType.OwningCollection:
						data = new LcmOwningCollection<ICmObject>(
							uowService,
							cmObjRepos,
							this, flid);
						((ILcmVectorInternal)data).LoadFromDataStoreInternal(customPropertyElement, loadingServices.m_objIdFactory);
						break;
					case CellarPropertyType.OwningSequence:
						data = new LcmOwningSequence<ICmObject>(
							uowService,
							cmObjRepos,
							this, flid);
						((ILcmVectorInternal)data).LoadFromDataStoreInternal(customPropertyElement, loadingServices.m_objIdFactory);
						break;
					case CellarPropertyType.ReferenceCollection:
						data = new LcmReferenceCollection<ICmObject>(
							uowService,
							cmObjRepos,
							this, flid);
						((ILcmVectorInternal)data).LoadFromDataStoreInternal(customPropertyElement, loadingServices.m_objIdFactory);
						break;
					case CellarPropertyType.ReferenceSequence:
						data = new LcmReferenceSequence<ICmObject>(
							uowService,
							cmObjRepos,
							this, flid);
						((ILcmVectorInternal)data).LoadFromDataStoreInternal(customPropertyElement, loadingServices.m_objIdFactory);
		...
		/// </summary>
		int ICmObjectOrIdInternal.GetHvo(IdentityMap identityMap)
		{
			return Hvo;
		}

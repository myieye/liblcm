...
{
	/// <summary>
	/// A common base class for all LCM backend providers.
	/// </summary>
	internal abstract partial class BackendProvider : IDataSetup, IDataReader, IDataStorer
	{
		...
		protected readonly ILcmUI m_ui;
		...
		protected readonly ILcmDirectories m_dirs;
		protected readonly LcmSettings m_settings;
		...
		{
			if (IsDisposed)
				throw new ObjectDisposedException("'BackendProvider' in use after being disposed.");
		}
		...
		protected virtual bool StartupInternalWithDataMigrationIfNeeded(IThreadedProgress progressDlg)
		{
			...
			if (currentDataStoreVersion > ModelVersion)
				throw new LcmNewerVersionException(Properties.Resources.kstidProjectIsForNewerVersionOfFw);
		...
		private void DoPort(IFwMetaDataCacheManaged mdc, IdentityMap sourceIdentityMap)
		{
			...
			// Instantiate core LCM objects in new system.
...
#pragma warning restore 219
			...
			IThreadedProgress progressDlg)
		{
			...
			// 3. Startup source BEP, but without instantiating any LCM objects (surrogates, are loaded).
			using (LcmCache sourceCache = LcmCache.CreateCacheFromExistingData(sourceDataStore.ProjectId,
		...
		///// </remarks>
		///// ------------------------------------------------------------------------------------
		//public void PortLanguageProject(IProjectIdentifier projectId, LcmCache sourceCache, int portVersion)
		//{
		//    if (sourceCache == null) throw new ArgumentNullException("sourceCache");
